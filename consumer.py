...
def put_object(method, to_bucket, json_object, file_name):
    try:
        print(f'{json_object["type"][:-1].capitalize()}ing {json_object["widgetId"]} in {to_bucket} using {method}')
        ...
        if json_object['type'] == 'create':
            ...
            f.close()
            to_resource.Object(to_bucket, f'{json_object["owner"]}/{json_object["widgetId"]}').upload_file(file_name)
        elif json_object['type'] == 'delete':
            to_resource.Object(to_bucket, f'{json_object["widgetId"]}').delete()
    except:
